...
@section('content')
    ...
    <div style="margin-left: 80px;margin-right: 80px;margin-bottom: 80px;margin-top: 40px;">
        {!! Form::model($model,[ 'action' => array('BlogRecordController@update', $model->id),
                                'files' => true,
                                'method' => 'post'])
                                 !!}
        ...
        @endif
        <div>{!!  Form::text('title',$model->title,["style"=>"width:400px"]) !!}</div>
        ...
        <div>{!! Form::submit('Save',["class"=>"btn btn-lg btn-success"]) !!}</div>
        {!! Form::close() !!}
    </div>
@endsection
